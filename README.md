...
2. **自宅の今（SwitchBot・任意）** — 自宅の温湿度計の現在値を表示。タップで過去の推移（24時間/3日/7日/全期間）を確認
3. **夜間グラフ** — 気温（左軸・暖色）× 湿度（右軸・寒色）の二軸折れ線。各時刻に天気アイコンと降水確率を小さく表示
4. **明日以降テーブル** — 気温上下限・湿度上下限・天候・降水確率を1日1行で
...
自宅の SwitchBot 温湿度計の現在値・履歴を表示できます。ブラウザから SwitchBot API は
直接呼べない（CORS 未対応・鍵の秘匿・履歴API無し）ため、**GitHub Actions が15分ごとに
取得し `switchbot-data` ブランチの `switchbot.json` に蓄積**、サイトはそれを読みます。
セットアップ（トークン取得・Secrets 登録など）は [`docs/switchbot.md`](docs/switchbot.md) を参照。
