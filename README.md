...
当日19時〜翌朝6時の「今夜」に絞って、**気温と湿度**を一目で把握するための天気予報アプリ。
...
ヘッダーのベルボタンで、**毎日17時に今夜（19〜翌6時）の気温・湿度・天気を通知**する機能をオンにできます（Web通知 API + Service Worker）。
バックエンドを持たない静的サイトのため、**アプリ（タブ／インストール済みPWA）が開いている間の best-effort** 動作です。17時台にアプリが開いていれば当日分を1回通知します。初回オン時にブラウザの通知許可を求めます。

## 「今夜」ウィンドウのロジック

- 現在が **6時以降** → 今夜 = 当日19:00 〜 翌日06:00
- 現在が **6時より前**（深夜〜早朝）→ 進行中の夜 = 前日19:00 〜 当日06:00

時刻の並びは 19, 20, …, 23, 0, 1, …, 6。この範囲で毎時データを絞り込み、気温・湿度の min/max を算出します。
